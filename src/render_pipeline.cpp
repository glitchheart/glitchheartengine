namespace rendering
{
    ...
    static void set_shader_values(Material &material, Shader &shader, Renderer *renderer)
    {
        ...
        {
            if(shader.uniform_buffers[i].mapping_type == UniformBufferMappingType::POINT ||
...
// Remember to free
    ...
    static size_t get_ubo_base_offset(ValueType value_type)
    {
        ...
        case ValueType::STRUCTURE:
        {
            // @Incomplete: Need to handle
            // Handled by finding structure index
            // and looking through each type recursively
            assert(false);
            return 0;
        }
        break;
    ...
    static size_t get_ubo_base_offset(i32 struct_index, Renderer *renderer)
    {
        ...
        {
            offset += get_ubo_base_offset(structure.uniforms[i].type);
        }
        return offset;
    }
    ...
    static void update_lighting_ubo(Renderer *renderer)
    {
        ...
        for(i32 light_index = 0; light_index < renderer->render.dir_light_count; light_index++)
        {
            ...
            //                            , 0, 0, light_index, light.direction
            //                            , renderer);
            // add_ubo_array_update_value(dirlight_update, dirlight_ubo
            //                            , 0, 1, light_index, light.ambient
            //                            , renderer);
            // add_ubo_array_update_value(dirlight_update, dirlight_ubo
            //                            , 0, 2, light_index, light.diffuse
            //                            , renderer);
            // add_ubo_array_update_value(dirlight_update, dirlight_ubo
            //                            , 0, 3, light_index, light.specular
            //                            , renderer);

            add_ubo_update_value(dirlight_update, dirlight_ubo
                                       , 0, 0, light.direction
                                       , renderer);
            add_ubo_update_value(dirlight_update, dirlight_ubo
                                       , 0, 1, light.ambient
