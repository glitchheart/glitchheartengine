...
void main()
{
	...
	vs_out.posWorld  = (translate * rotMat * modelMatrix * vec4(position, 1)).xyz;
	...
	vec3 viewPos = (viewMatrix * translate * rotMat * modelMatrix * vec4(position, 1)).xyz;
	...
	vs_out.normal = mat3(transpose(inverse(viewMatrix * translate * rotMat * modelMatrix))) * vertexNormal;
	vs_out.uv = uv;
	vs_out.color = color;
}