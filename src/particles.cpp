...
void emit_particle(ParticleSystemInfo &particle_system, i32* alive_buf, i32* count, RandomSeries& entropy, i32 emitted_count)
{
    ...
    if(particle_system.attributes.start_size_type == StartParameterType::RANDOM_BETWEEN_TWO_CONSTANTS)
    {
        ...
        Vec2_4x size_val = Vec2_4x(v, v);
        particle_system.particles.size[original_index] = size_val;
        particle_system.particles.start_size[original_index] = size_val;
    }
    else
    {
        particle_system.particles.size[original_index] = Vec2_4x(particle_system.attributes.size.constant.start_size);
        particle_system.particles.start_size[original_index] = Vec2_4x(particle_system.attributes.size.constant.start_size);
    }
