		...
		if (particle_system.running)
		{
			...
			if (particle_system.emitting)
			{
				...
				if (particle_system.particles_cumulative >= 1.0f)
				{
				    ...
				    particle_system.particles_cumulative = particle_system.particles_cumulative - 1.0f;
				}
				else // @Note(Niels): Otherwise just find the round number of particles to emit
				{
				    //new_particles = math::round(per_frame);
				    //new_particles = particle_system.attributes.particles_per_second;
				}

				if (new_particles >= 1)
				{
				    particle_system.time_spent = particle_system.time_spent - 1.0;
				}
